...
function UrgentRow({ item, patient, onView, onAction }) {
  ...
  const tc = tagColors[item.tagColor] || tagColors.navy;
  return (
    <div style={{
      ...
      {patient
        ...
        : <div style={{ width: 40, height: 40, borderRadius: 8, background: C.creamWarm, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>⊟</div>}
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
          {patient && <span style={{ fontFamily: fonts.body, fontWeight: 700, fontSize: 14, color: C.text }}>{patient.name}</span>}
          {patient && patient.age != null && <span style={{ fontSize: 12, color: C.textMuted }}>· {patient.age}</span>}
          {patient && patient.conditions && <ConditionChips conditions={patient.conditions.slice(0, 2)} />}
      ...
      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
        {patient && <Button variant="ghost" size="sm" onClick={() => onView(patient)}>View</Button>}
...
function RevenueBarChart({ data, height = 140 }) {
  const max = Math.max(1, ...data.map(d => d.amount)); // avoid /0 → NaN when no sales yet
...
function DashboardScreen({ onNavigate, onAction }) {
  // Real dashboard payload from the backend (window.PORTAL_DASHBOARD), or null in
  // demo mode → fall back to the bundled sample data. inr() takes rupees, so
  // backend paise are divided by 100.
  const RD = window.PORTAL_DASHBOARD || null;
  const rupees = (paise) => Math.round((paise || 0) / 100);

  const urgentSource = RD ? RD.urgent : URGENT;
  const urgentItems = urgentSource.map(u => ({ item: u, patient: u.patientId ? PATIENTS.find(p => p.id === u.patientId) : null }));
  const redCount = urgentSource.filter(u => u.tagColor === 'red').length;
  const activityFeed = RD ? RD.activity : ACTIVITY;
  const rev7 = RD ? RD.revenue7d.map(d => ({ ...d, amount: rupees(d.amount) })) : REV_7DAYS;

  const pharmacyName = (RD && RD.pharmacyName) || (window.PHARMACY && window.PHARMACY.name) || 'Sharma Medical Store';
  const now = new Date();
  ...
  return (
    <div>
      ...
      <PageHeader
        title={`${greeting}`}
        subtitle={`${dateStr} · ${timeStr} · ${pharmacyName}`}
        right={
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, padding: '0 24px' }}>
        <StatCard
          ...
          value={RD ? String(RD.pendingRefills) : "12"}
          sub={RD ? `${RD.criticalRefills} critical · ${RD.pendingRefills - RD.criticalRefills} awaiting confirm` : "3 critical · 9 awaiting confirm"}
          accent="red"
          badge={RD ? (RD.criticalRefills > 0 ? `${RD.criticalRefills} URGENT` : null) : "3 URGENT"}
          onClick={() => onNavigate('refills')}
        />
        <StatCard
          label="Today's orders"
          value={RD ? String(RD.ordersToday) : "23"}
          sub={RD ? `${inr(rupees(RD.revenueToday))} · ${RD.readyRefills} ready for pickup` : inr(18400) + ' · 14 ready for pickup'}
          accent="navy"
          onClick={() => onNavigate('refills')}
        />
        <StatCard
          label="Active patients"
          value={RD ? String(RD.patientCount) : "247"}
          sub={RD ? `+${RD.newThisWeek} this week` : "+1 this week · 12 inactive"}
          accent="sage"
          onClick={() => onNavigate('patients')}
        />
        <StatCard
          label="Revenue this month"
          value={RD ? inr(rupees(RD.revenueMonth)) : inr(342850)}
          sub={RD ? `${RD.salesToday} counter sales today` : "May 1 – May 11"}
          accent="coral"
        ...
        <Card>
          ...
          <div style={{ maxHeight: 560, overflowY: 'auto' }}>
            {activityFeed.length === 0
              ? <div style={{ padding: '24px 4px', color: C.textMuted, fontSize: 13 }}>No activity yet today.</div>
              : activityFeed.map(e => <ActivityRow key={e.id} event={e} />)}
          </div>
        </Card>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16, padding: '16px 24px 24px' }}>
        <Card>
          ...
          />
          <RevenueBarChart data={rev7} />
        </Card>

        <Card>
          <SectionHeader title="At a glance" />
          <div style={{ padding: '4px 0' }}>
            <KpiRow label="Avg order value" value={RD ? inr(rupees(RD.avgOrderValue)) : inr(1640)} />
            <KpiRow label="Pickups ready" value={RD ? `${RD.readyRefills} / ${RD.pendingRefills + RD.readyRefills}` : "14 / 23"} />
            <KpiRow label="Adherence avg (30d)" value={RD ? (RD.adherenceAvg == null ? '—' : RD.adherenceAvg + '%') : "79%"} />
            <KpiRow label="Low-stock items" value={RD ? String(RD.lowStock) : "3"} warn={RD ? RD.lowStock > 0 : true} />
            <KpiRow label="At-risk patients" value={RD ? String(RD.atRiskCount) : "9"} warn />
            <KpiRow label="New patients this week" value={RD ? `+${RD.newThisWeek}` : "+1"} trend={RD ? (RD.newPatientName || '') : "Aarti Verma"} last />
          </div>
        </Card>
      </div>
    </div>
